...
public class InfoExchange {
	...
	public static void OrderFiller(Order order,
			HashMap<Integer, String> orderMap) {
		order.OrderType = orderMap.get(map.get("OrderType"));
		order.SenderCompID = Integer.parseInt(orderMap.get(map
				.get("SenderCompID")));
	...
	public static String OrderFixConstructor(Order order) {
		...
		String fixOrderBody = String.format(fixOrder, MsgType, SenderCompID,
				SendingTime, ClOrdID, Side, OrderQty, Price, Symbol, OrderType,
				TimeInForce, TransacTime, MaturityMonthYear, MaturityDay,
				ExecInst);
	...
	public static String ACKDeparser(ACK ack) {
		...
		int len = fixOrderBody.length() + fixACKBody.length()
				+ fixTail.length();
		String prefix = String.format("8=FIX.4.2^A9=%d^A", len);
	...
	public static ExeReport ExeParser(String fixMsg) {
		...
		if (exeMap.containsKey(map.get("ExecTranType")))
			exeReport.ExecTranType = Integer.parseInt(exeMap.get(map
					.get("ExecTranType")));
		if (exeMap.containsKey(map.get("LastPx")))
			exeReport.LastPx = Double
					.parseDouble(exeMap.get(map.get("LastPx")));
		if (exeMap.containsKey(map.get("LastShares")))
			exeReport.LastShares = Integer.parseInt(exeMap.get(map
					.get("LastShares")));
		if (exeMap.containsKey(map.get("OrdStatus")))
			exeReport.OrdStatus = Integer.parseInt(exeMap.get(map
					.get("OrdStatus")));
		if (exeMap.containsKey(map.get("OrderID")))
			exeReport.OrderID = Integer
					.parseInt(exeMap.get(map.get("OrderID")));
		if (exeMap.containsKey(map.get("ExecID")))
			exeReport.ExecID = Integer.parseInt(exeMap.get(map.get("ExecID")));
		if (exeMap.containsKey(map.get("ExecTranType")))
			exeReport.ExecTranType = Integer.parseInt(exeMap.get(map
					.get("ExecTranType")));
		if (exeMap.containsKey(map.get("LeavesQty")))
			exeReport.LeavesQty = Integer.parseInt(exeMap.get(map
					.get("LeavesQty")));
		if (exeMap.containsKey(map.get("lastCapacity")))
			exeReport.LastCapacity = Integer.parseInt(exeMap.get(map
					.get("lastCapacity")));
		return exeReport;
	}
	...
	public static String ExeReportDeparser(ExeReport exeReport) {
		...
		String fixExeBody = String.format(fixExeInfo, AvgPx, CumQty, ExecID,
				ExecTransType, LastCapacity, LastPx, LastShares, OrderID,
				OrdStatus, ExecType, LeavesQty);
		int len = fixOrderBody.length() + fixExeBody.length()
				+ fixTail.length();
		String prefix = String.format("8=FIX.4.2^A9=%d^A", len);
